...
final class RestaurantsViewController: UIViewController {
	...
	override func viewDidLayoutSubviews() {
		...
		tableView.contentInset = UIEdgeInsets(
			top: headerView.frame.height + filterBar.frame.height + searchView.frame.height,
			left: 0,
			bottom: 0,
			right: 0
		)
	...
	override func viewDidAppear(_ animated: Bool) {
		...
		filterBarUpperBorder = headerView.frame.height - filterBar.frame.height - searchView.frame.height + view.safeAreaInsets.top
...
extension RestaurantsViewController: UIScrollViewDelegate {
	...
	private func moveFilterBar(by offset: CGFloat, direction: ScrollDirection) {
		...
		case .down:
			...
			if let filterBarBottomBorder = filterBarBottomBorder,
